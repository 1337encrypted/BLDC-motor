...
class BLDCPulseCalculator {
  ...
  TaskHandle_t motorSpeedTaskHandle;
  
  public:
  inline BLDCPulseCalculator(gpio_num_t wavePin = GPIO_NUM_NC, uint8_t motorId = -1);
...
void BLDCPulseCalculator::begin(const BaseType_t app_cpu) {
  ...
  const char* TAG = "BLDCPulseCalculator::begin";
